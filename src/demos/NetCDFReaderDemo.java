...
public class NetCDFReaderDemo extends BeanImageDemo
{
	...
	static void netCDFDemo(int cellWidth, int cellHeight, boolean show)
	{
		...
		String[] fieldNames = FieldUtils.getInstanceFieldNames(FieldUtils.getAnnotatedFields(AllFlavorBean.class, ParsedField.class));
		...
		{
			imagers.add(ImagerFactory.quickFactory(beans, 12, true, true, fieldNames[i], AllFlavorBean.class,
					gradCols, boolCols));
			objPan = ObjectArrayPanelFactory.buildPanel(
					imagers.get(i), fieldNames[i], true, 0, 0, ptSize);
