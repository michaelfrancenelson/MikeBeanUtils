...
public class RandomBeanBuilder extends AnnotatedBeanReader 
{
	...
	@Deprecated	public static <T> List<List<T>> randomFactory(Class<T> clazz, int nRow, int nCol, int min, int max) throws IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException { return randomFactory(clazz, nRow, nCol, min, max, r); }
	@Deprecated public static <T> T randomFactory(Class<T> clazz, int min, int max) throws IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException { return randomFactory(clazz, min, max, r); }
	@Deprecated public static <T> List<T> randomFactory(Class<T> clazz, int n, int min, int max) throws IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException { return randomFactory(clazz, n, min, max, r); }
	 ...
	 */
	public static <T> List<List<T>> randomFactory(
			Class<T> clazz, int nRow, int nCol, int min, int max, RandomStream rs) 
	{
		List<List<T>> l = new ArrayList<>();
		for (int i = 0; i < nCol; i++)
			l.add(randomFactory(clazz, nRow, min, max, rs));
		return l;
